...
- Template Wiki (Rider-first): [Chinese Home](https://github.com/alkaid616/ElectricDragonKing/wiki/Home) | [English Home](https://github.com/alkaid616/ElectricDragonKing/wiki/Home-EN).
...
`dotnet new ritsulibmod -n MyMod` generates a project called `MyMod` and renames `ElectricDragonKing`, sample class names, sample resource file names, resource folders, manifest names, namespaces, and localization IDs to match the new name.

### Option B: manual copy

1. Copy the whole directory and rename it to your mod name.
2. Edit `ElectricDragonKing.json` and update `id`, `name`, `author`, and `description`.
3. Edit `ElectricDragonKingCode/Entry.cs` and update `ModId`.
4. For a full rename, also update the project name and namespace in `.csproj`, `.sln`, and `project.godot`.
5. Rename the resource directory `ElectricDragonKing/` to your `ModId`, then update the related `Entry.ResPath` paths in code.
...
> **`PackageReference` in `.csproj` only controls compile-time resolution; `dependencies` in `ElectricDragonKing.json` is what the game loader checks at runtime. The template syncs the `STS2-RitsuLib` dependency version to the resolved NuGet version during build, but `min_game_version` still needs manual review.**
...
1. After build, confirm `dependencies[STS2-RitsuLib].version` in `ElectricDragonKing.json` has been synced to the resolved `STS2.RitsuLib` version.
...
| Command | Behavior |
|---|---|
| `dotnet build .\ElectricDragonKing.csproj` | Full build: compile + `CopyMod` + `ExportPCK` |
...
- **`CopyMod`**: copies the DLL and manifest to the game's `mods/ElectricDragonKing` directory.
...
```text
ElectricDragonKing/
├── ElectricDragonKingCode/   # C# source
├── ElectricDragonKing/       # Godot resources, localization, and placeholder scenes
├── ElectricDragonKing.csproj
├── ElectricDragonKing.json   # Mod manifest
├── project.godot
└── local.props.template
```

`res://ElectricDragonKing/...` is the Godot/PCK resource path, mapping to the repository resource directory `ElectricDragonKing/`, **not** to the C# namespace. When you create a project from the NuGet template, these directory names, file names, and namespaces are renamed consistently to match the new mod name.
...
| Property | Value |
|---|---|
| Type | `ElectricDragonKingCharacter` |
| Expected ID | `ELECTRIC_DRAGON_KING_CHARACTER_ELECTRIC_DRAGON_KING_CHARACTER` |
| Starter deck | 4 × `ElectricDragonKingStrike`, 4 × `ElectricDragonKingDefend`, 1 × `ElectricDragonKingRelic` |
...
|---|---|---|
| `ElectricDragonKingStrike` (attack) | character card pool | `ELECTRIC_DRAGON_KING_CARD_ELECTRIC_DRAGON_KING_STRIKE` |
| `ElectricDragonKingDefend` (skill) | character card pool | `ELECTRIC_DRAGON_KING_CARD_ELECTRIC_DRAGON_KING_DEFEND` |
| `ElectricDragonKingRelic` | `ElectricDragonKingRelicPool` | `ELECTRIC_DRAGON_KING_RELIC_ELECTRIC_DRAGON_KING_RELIC` |

### Static placeholder assets

**Images** (`res://ElectricDragonKing/images/...`):

- `cards/ElectricDragonKingStrike.png`, `cards/ElectricDragonKingDefend.png`: sample card art.
- `relics/ElectricDragonKingRelic.png`: sample relic icon.
- `characters/ElectricDragonKing_character_*.png`: character icons, select art, map marker, and energy icons.

**Scenes** (`res://ElectricDragonKing/scenes/characters/...`):

| Scene | Purpose | Placeholder structure |
|---|---|---|
| `ElectricDragonKing_character.tscn` | Combat character | `%Visuals`, `%Bounds`, `%IntentPos`, `%CenterPos`, `%TalkPos` |
| `ElectricDragonKing_energy_counter.tscn` | Energy counter | `%EnergyVfxBack`, `%Layers`, `%RotationLayers`, `%EnergyVfxFront`, `Label` |
| `ElectricDragonKing_merchant.tscn` | Merchant | — |
| `ElectricDragonKing_rest_site.tscn` | Rest site | `%ControlRoot`, `%SelectionReticle`, `%Hitbox`, `%ThoughtBubbleRight`, `%ThoughtBubbleLeft` |
| `ElectricDragonKing_character_select_bg.tscn` | Character select background | — |
...
`ElectricDragonKing.json` is the mod manifest. The game loader reads it at startup to identify the mod, check dependencies, and decide whether to load. Full example:

```json
{
  "id": "ElectricDragonKing",
  "name": "ElectricDragonKing",
  "pck_name": "ElectricDragonKing",
  "author": "Author",
